...
function handleMouseOver(e) {
  ...
  if (element.id === 'synapse-overlay' || element.id === 'synapse-notification') {
    return;
  }
...
function extractContent(element) {
  ...
  // Get videos
  const videos = Array.from(element.querySelectorAll('video, iframe[src*="youtube"], iframe[src*="vimeo"]')).map(v => ({
...
async function categorizeContent(content) {
  try {
    ...
    if (content.links.some(link => link.href.includes('amazon') || link.href.includes('product'))) return 'Products';
