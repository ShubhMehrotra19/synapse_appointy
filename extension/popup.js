...
async function syncAuthFromTabs() {
  try {
    // First, try to get all tabs and filter for localhost:3000
    const allTabs = await chrome.tabs.query({});
    const synapseTabs = allTabs.filter(tab => 
...
async function checkAuth() {
  // First check storage
  const result = await chrome.storage.sync.get(['synapse_token', 'synapse_user']);
  ...
  if (!isAuthenticated) {
    ...
    if (synced) {
      // Re-check after sync
      const newResult = await chrome.storage.sync.get(['synapse_token', 'synapse_user']);
  ...
  chrome.tabs.create({ url: 'http://localhost:3000/login' }, (tab) => {
    ...
    const listener = (tabId, changeInfo, updatedTab) => {
      if (tabId === tab.id && changeInfo.status === 'complete') {
        if (updatedTab.url && (updatedTab.url.includes('auth-callback') || updatedTab.url.includes('dashboard'))) {
...
document.getElementById('capture-btn')?.addEventListener('click', async () => {
  ...
  try {
    ...
    if (errorMsg.includes('Receiving end does not exist')) {
      ...
      errorMsg = 'Content script did not respond. Please refresh the page and try again.';
    }
